...
const defaultSeedData = {
    products: [
        { code: 'PRD-001', name: 'កាហ្វេអាល់ប៊ីកា (Arabica Coffee Beans 1kg)', category: 'ភេសជ្ជៈ', cost: 12.50, price: 18.00, qty: 25, minAlert: 10 },
        { code: 'PRD-002', name: 'ទឹកដោះគោឆៅ (Fresh Milk 1L)', category: 'ភេសជ្ជៈ', cost: 1.80, price: 2.50, qty: 4, minAlert: 10 },
        { code: 'PRD-003', name: 'ស្ករស (White Sugar 1kg)', category: 'គ្រឿងទេស', cost: 0.90, price: 1.30, qty: 50, minAlert: 15 },
        { code: 'PRD-004', name: 'កែវក្រដាស 16oz (Paper Cups 50pcs)', category: 'សម្ភារៈ', cost: 2.20, price: 3.50, qty: 0, minAlert: 5 },
        { code: 'PRD-005', name: 'តែបៃតងជប៉ុន (Matcha Green Tea 500g)', category: 'ភេសជ្ជៈ', cost: 15.00, price: 22.00, qty: 8, minAlert: 5 },
        { code: 'PRD-006', name: 'ស៊ីរ៉ូរសជាតិវ៉ានីឡា (Vanilla Syrup 750ml)', category: 'គ្រឿងទេស', cost: 6.50, price: 9.50, qty: 3, minAlert: 5 }
    ],
    stockInLogs: [
    ...
    suppliers: [
        { id: 1, name: 'ក្រុមហ៊ុន ភ្នំពេញ កាហ្វេ', phone: '012 888 999', address: 'រាជធានីភ្នំពេញ' },
        { id: 2, name: 'ផ្សារអូរឫស្សី ស្តង់A12', phone: '097 777 666', address: 'ខណ្ឌ៧មករា ភ្នំពេញ' }
    ],
    customers: [
        ...
        google.script.run
            .withSuccessHandler((response) => {
                if (response && response.success) {
                    ...
                    state.stockInLogs = response.data.stockInLogs || [];
                    state.stockOutLogs = response.data.stockOutLogs || [];
                    state.suppliers = response.data.suppliers || [];
                    state.customers = response.data.customers || [];
                    saveToLocalStorage();
                    refreshCurrentUI();
                    showToast('បានធ្វើបច្ចុប្បន្នភាពទិន្នន័យជោគជ័យ!', 'success');
                } else {
                    fallbackLocalStorage();
                }
            })
            .withFailureHandler((err) => {
                showToast('ពុំអាចភ្ជាប់ទៅ Google Sheets បានទេ! ប្រើប្រាស់ Local Data', 'warning');
                fallbackLocalStorage();
            })
            .getInitialData();
    } else {
...
function fallbackLocalStorage() {
    const localProds = localStorage.getItem('km_stock_products');
    if (localProds) {
        state.products = JSON.parse(localProds);
        state.stockInLogs = JSON.parse(localStorage.getItem('km_stock_in') || '[]');
        state.stockOutLogs = JSON.parse(localStorage.getItem('km_stock_out') || '[]');
        state.suppliers = JSON.parse(localStorage.getItem('km_suppliers') || '[]');
        state.customers = JSON.parse(localStorage.getItem('km_customers') || '[]');
    } else {
...
function saveToLocalStorage() {
    localStorage.setItem('km_stock_products', JSON.stringify(state.products));
    localStorage.setItem('km_stock_in', JSON.stringify(state.stockInLogs));
    localStorage.setItem('km_stock_out', JSON.stringify(state.stockOutLogs));
    localStorage.setItem('km_suppliers', JSON.stringify(state.suppliers));
    localStorage.setItem('km_customers', JSON.stringify(state.customers));
}
...
function syncToGoogleSheets(action, payload, successCallback) {
    ...
    const gasUrl = localStorage.getItem('km_gas_url');
    ...
    if (state.isGoogleAppsScript) {
        ...
        .catch(() => {
            showToast('ទិន្នន័យបានរក្សាទុកក្នុង Local Storage (Offline Mode)', 'info');
        });
    } else {
...
function setupEventListeners() {
    ...
    document.addEventListener('click', (e) => {
        ...
        const navEl = e.target.closest('[data-page], a[href^="#"]');
        if (navEl) {
    ...
    if (btnSync) btnSync.addEventListener('click', (e) => { e.preventDefault(); loadData(); });
    ...
    if (stockOutProdSelect) {
        stockOutProdSelect.addEventListener('change', (e) => {
            ...
            if (p) document.getElementById('stockOutFormUnitPrice').value = p.price;
        });
    }
    ...
    if (stockInProdSelect) {
        stockInProdSelect.addEventListener('change', (e) => {
            const p = state.products.find(prod => prod.code === e.target.value);
            if (p) document.getElementById('stockInFormUnitPrice').value = p.cost;
        });
    }
...
function renderPage(pageId) {
    ...
    const titles = {
        ...
        'contacts': 'អតិថិជន & អ្នកផ្គត់ផ្គង់ (Contacts)',
...
function renderDashboardKPIs() {
    try {
        ...
        const totalValue = (state.products || []).reduce((sum, p) => sum + ((p.cost || 0) * (p.qty || 0)), 0);
        const lowStockCount = (state.products || []).filter(p => (p.qty || 0) > 0 && (p.qty || 0) <= (p.minAlert || 5)).length;
        const outOfStockCount = (state.products || []).filter(p => (p.qty || 0) === 0).length;
...
function renderLowStockTable() {
    const alerts = state.products.filter(p => p.qty <= p.minAlert);
    const tbody = document.getElementById('lowStockTableBody');
    tbody.innerHTML = '';

    if (alerts.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted p-3"><i class="fa-solid fa-circle-check text-success"></i> គ្មានស្តុកដែលត្រូវព្រមានទេ!</td></tr>`;
        return;
    }
    ...
    alerts.forEach(p => {
        ...
        tr.innerHTML = `
            <td data-label="កូដ"><code>${p.code}</code></td>
            <td data-label="ឈ្មោះទំនិញ"><strong>${p.name}</strong></td>
            <td data-label="ប្រភេទទំនិញ"><span class="badge badge-info">${p.category}</span></td>
            <td data-label="ស្តុកបច្ចុប្បន្ន"><span class="badge ${badgeClass}">${p.qty}</span></td>
            <td data-label="កម្រិតរ៉កកាត">${p.minAlert}</td>
            <td data-label="សកម្មភាព">
                <button class="btn btn-action-stockin" onclick="quickStockInForCode('${p.code}')">
                    <i class="fa-solid fa-plus-circle"></i> <span>ថែមស្តុក</span>
                </button>
            </td>
        `;
...
function renderRecentActivities() {
    const logs = [];
    state.stockInLogs.forEach(l => logs.push({ type: 'IN', date: l.date, text: `នាំចូលស្តុក: ${l.name} (+${l.qty})`, amount: `$${l.total}` }));
    state.stockOutLogs.forEach(l => logs.push({ type: 'OUT', date: l.date, text: `លក់ចេញ: ${l.name} (-${l.qty})`, amount: `$${l.total}` }));
    ...
    if (paginated.length === 0) {
        ...
        paginated.forEach((p, idx) => {
            ...
            if (tbody) {
                const tr = document.createElement('tr');
                tr.innerHTML = `
                    <td data-label="NO" class="text-center text-muted">${idx + 1}</td>
                    <td data-label="កូដទំនិញ/បាកូដ"><code>${p.code}</code></td>
                    <td data-label="ឈ្មោះទំនិញ"><strong>${p.name}</strong></td>
                    <td data-label="ប្រភេទទំនិញ"><span class="badge badge-info">${p.category}</span></td>
                    <td data-label="ឯកតា" class="text-center">${p.unit || 'កញ្ចប់'}</td>
                    <td data-label="តម្លៃដើម" class="text-end">$${Number(p.cost).toFixed(2)}</td>
                    <td data-label="តម្លៃលក់" class="text-end"><strong class="text-primary">$${Number(p.price).toFixed(2)}</strong></td>
...
function handleProductFormSubmit(e) {
    ...
    const productObj = { code, name, category, cost, price, qty, minAlert };
...
function initStockInPanel() {
    ...
    if (supSelect) {
        supSelect.innerHTML = '<option value="ទូទៅ">-- ពុំមាន / ទូទៅ --</option>' +
    ...
    if (prodSelect) {
        prodSelect.innerHTML = '<option value="">-- ជ្រើសរើសមុខទំនិញ --</option>' +
            state.products.map(p => `<option value="${p.code}">[${p.code}] ${p.name} (ស្តុក: ${p.qty})</option>`).join('');
    }
...
function renderStockInCartTable() {
    ...
    if (state.stockInCart.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted p-4">មិនទាន់មានទំនិញក្នុងកន្ត្រកទិញឡើយ</td></tr>`;
...
function addToStockInCart() {
    ...
    if (existing) {
        ...
        state.stockInCart.push({ code: prod.code, name: prod.name, qty, cost });
    }
...
function completeStockInTx() {
    ...
    const supplier = document.getElementById('stockInSupplierFormSelect').value || 'ទូទៅ';
...
function renderStockInTable() {
    ...
    state.stockInLogs.forEach(l => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="ចំនួនបន្ថែម" class="text-center"><span class="badge badge-success">+${l.qty}</span></td>
            <td data-label="តម្លៃទិញចូល" class="text-end">$${Number(l.cost).toFixed(2)}</td>
            <td data-label="សរុបទឹកប្រាក់" class="text-end"><strong class="text-success">$${Number(l.total).toFixed(2)}</strong></td>
            <td data-label="សកម្មភាព" class="text-center">
...
function renderStockOutCartTable() {
    ...
    if (state.stockOutCart.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted p-4">មិនទាន់មានទំនិញក្នុងកន្ត្រកលក់ឡើយ</td></tr>`;
...
function addToStockOutCart() {
    ...
    if (existing) {
        ...
        state.stockOutCart.push({ code: prod.code, name: prod.name, qty, price });
    }
...
function completeStockOutTx() {
    ...
    const receiptTotal = receiptItems.reduce((sum, i) => sum + (i.qty * i.price), 0);
    ...
    openReceiptModal(invNo, 'StockOut', date, customer, receiptItems, receiptTotal);
}
...
function renderStockOutTable() {
    ...
    state.stockOutLogs.forEach(l => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="កូដទំនិញ"><code>${l.code}</code></td>
            <td data-label="ឈ្មោះទំនិញ"><strong>${l.name}</strong></td>
            <td data-label="ចំនួន" class="text-center"><span class="badge badge-warning">-${l.qty}</span></td>
            <td data-label="តម្លៃលក់" class="text-end">$${Number(l.price).toFixed(2)}</td>
            <td data-label="TOTAL" class="text-end"><strong class="text-primary">$${Number(l.total).toFixed(2)}</strong></td>
            <td data-label="អតិថិជន">${l.customer || 'អតិថិជនទូទៅ'}</td>
            <td data-label="សកម្មភាព" class="text-center">
                <button class="btn btn-xs btn-outline-primary" onclick="reprintStockOutReceipt('${l.id}')" title="បោះពុម្ពវិក្កយបត្រ"><i class="fa-solid fa-print"></i></button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
function openReceiptModal(invNo, type, date, contactName, items, grandTotal) {
    ...
    items.forEach((item, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td class="text-center fw-bold" style="padding: 0.45rem 0; color: #0f172a;">${item.qty}</td>
...
function reprintStockOutReceipt(invNo) {
    ...
    const grandTotal = logs.reduce((sum, l) => sum + (l.total || (l.qty * l.price)), 0);
    openReceiptModal(invNo, 'StockOut', first.date, first.customer, items, grandTotal);
}
...
function renderContactsTables() {
    ...
    state.suppliers.forEach((s, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="សកម្មភាព" class="text-end">
                <button class="btn btn-xs btn-danger" onclick="deleteContact('supplier', ${idx})"><i class="fa-solid fa-trash"></i></button>
            </td>
        `;
    ...
    state.customers.forEach((c, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="សកម្មភាព" class="text-end">
                <button class="btn btn-xs btn-danger" onclick="deleteContact('customer', ${idx})"><i class="fa-solid fa-trash"></i></button>
            </td>
        `;
...
function openContactModal(type) {
    ...
    document.getElementById('contactModalTitle').textContent = type === 'supplier' ? 'បន្ថែមអ្នកផ្គត់ផ្គង់' : 'បន្ថែមអតិថិជន';
...
function handleContactSubmit(e) {
    ...
    if (type === 'supplier') {
        ...
        syncToGoogleSheets('saveSupplier', obj, () => {
            ...
            renderContactsTables();
            showToast('បានរក្សាទុកអ្នកផ្គត់ផ្គង់!', 'success');
        });
    } else {
        ...
        syncToGoogleSheets('saveCustomer', obj, () => {
            document.getElementById('contactModal').classList.remove('show');
            renderContactsTables();
            showToast('បានរក្សាទុកអតិថិជន!', 'success');
        });
    }
}

function deleteContact(type, idx) {
    if (confirm('តើអ្នកពិតជាចង់លុបព័ត៌មាននេះឬ?')) {
        if (type === 'supplier') state.suppliers.splice(idx, 1);
        else state.customers.splice(idx, 1);
        saveToLocalStorage();
        renderContactsTables();
        showToast('បានលុបព័ត៌មានទំនាក់ទំនង', 'success');
    }
}
...
function renderReportSection() {
    ...
    if (typeFilter === 'all' || typeFilter === 'IN') {
        state.stockInLogs.forEach(l => combined.push({
            date: l.date,
            type: 'STOCK_IN',
            code: l.code,
            name: l.name,
    ...
    if (typeFilter === 'all' || typeFilter === 'OUT') {
        state.stockOutLogs.forEach(l => combined.push({
            date: l.date,
            type: 'STOCK_OUT',
            code: l.code,
            name: l.name,
    ...
    combined.forEach(c => {
        ...
        tr.innerHTML = `
            ...
            <td data-label="តម្លៃឯកតា" class="text-end">$${Number(c.price).toFixed(2)}</td>
            <td data-label="សរុប ($)" class="text-end"><strong class="text-primary">$${Number(c.total).toFixed(2)}</strong></td>
...
function updateBadges() {
    document.getElementById('totalProductsBadge').textContent = state.products.length;
}

function exportProductsExcel() {
    try {
        let content = `\uFEFFកូដទំនិញ,ឈ្មោះទំនិញ,ប្រភេទទំនិញ,តម្លៃដើម ($),តម្លៃលក់ ($),ចំនួនក្នុងស្តុក,កម្រិតព្រមានស្តុក\n`;
        state.products.forEach(p => {
            const cleanName = (p.name || '').replace(/"/g, '""');
            const cleanCat = (p.category || '').replace(/"/g, '""');
            content += `"${p.code}","${cleanName}","${cleanCat}",${p.cost},${p.price},${p.qty},${p.minAlert}\n`;
        });
...
function loadSettingsUI() {
    const gasUrl = localStorage.getItem('km_gas_url') || '';
    const sheetId = localStorage.getItem('km_sheet_id') || '';
    const storeName = localStorage.getItem('km_store_name') || 'ហាងទំនិញ ខ្មែរ Pro';
    const currency = localStorage.getItem('km_currency') || '$';
    const defaultMinAlert = localStorage.getItem('km_default_min_alert') || '5';
...
function deleteCategoryFromSettings(catName) {
    ...
    let customCats = JSON.parse(localStorage.getItem('km_custom_categories') || '[]');
...
function renderSettingsProductsTable() {
    ...
    const filtered = state.products.filter(p => p.name.toLowerCase().includes(query) || p.code.toLowerCase().includes(query) || p.category.toLowerCase().includes(query));

    if (countText) countText.textContent = `ទំនិញសរុប៖ ${filtered.length} / ${state.products.length} ប្រភេទ`;
    tbody.innerHTML = '';

    if (filtered.length === 0) {
        tbody.innerHTML = '<tr><td colspan="7" class="text-center text-muted p-4"><i class="fa-solid fa-box-open me-2 fs-4"></i><br>ពុំទាន់មានទំនិញក្នុងស្តុកទេ</td></tr>';
        return;
    }
...
function saveSystemSettings() {
    ...
    localStorage.setItem('km_gas_url', gasUrl);
    localStorage.setItem('km_sheet_id', sheetId);
    localStorage.setItem('km_store_name', storeName);
    localStorage.setItem('km_currency', currency);
    localStorage.setItem('km_default_min_alert', defaultMinAlert);
        ...
        google.script.run
            .withSuccessHandler(res => {
                if (res && res.success) {
                    ...
                    saveToLocalStorage();
                    refreshCurrentUI();
                    showToast(`បានទាញយកទិន្នន័យពី Google Sheets ជោគជ័យ! (${state.products.length} ទំនិញ)`, 'success');
                    updateConnectionStatusDisplay();
                } else {
