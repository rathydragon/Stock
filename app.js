        ...
        google.script.run
            .withSuccessHandler((response) => {
                if (response && response.success) {
                    ...
                    const msg = (response && response.message) ? response.message : 'ពុំអាចទាញយកទិន្នន័យបានទេ';
                    showToast('កំហុស Google Sheets: ' + msg, 'danger');
                    fallbackLocalStorage();
                }
            })
            .withFailureHandler((err) => {
                const errMsg = err ? (err.message || err.toString()) : 'កំហុសមិនស្គាល់';
                showToast('ពុំអាចភ្ជាប់ទៅ Google Sheets បានទេ: ' + errMsg, 'danger');
                fallbackLocalStorage();
            })
            .getInitialData();
    } else {
...
function fallbackLocalStorage() {
    ...
    if (localProds) {
        ...
        state.suppliers = JSON.parse(safeStorage.getItem('km_suppliers') || '[]');
        state.customers = JSON.parse(safeStorage.getItem('km_customers') || '[]');
    } else {
