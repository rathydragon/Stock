...
function getLoggedUserStoreNames() {
    ...
    // Common default keywords & general tags so stores can access general & store owner data
    names.add('ទូទៅ');
    names.add('general');
    names.add('');
    names.add('ម្ចាស់ហាង');
    names.add('store');
    names.add('store owner');
...
function isStoreRoleUser() {
    ...
    const r = (state.currentUser.role || '').trim().toLowerCase();
    const u = (state.currentUser.username || '').trim().toLowerCase();
    return r === 'store' || r === 'seller' || r.includes('store') || r.includes('ម្ចាស់ហាង') || u === 'store';
}
...
function isCustomerRoleUser() {
    ...
    const r = (state.currentUser.role || '').trim().toLowerCase();
    const u = (state.currentUser.username || '').trim().toLowerCase();
    if (r === 'store' || r === 'seller' || r.includes('store') || r.includes('ម្ចាស់ហាង') || u === 'store' || u === 'admin' || r === 'admin') {
        return false;
    }
    return r === 'customer' || r.includes('អតិថិជន');
}
...
function getStoreFilteredProducts() {
    ...
    if (!state.currentUser) return state.products;

    if (isCustomerRoleUser()) {
        const myCustNames = getLoggedCustomerNames();
        return state.products.filter(p => {
            if (!p) return false;
            if (p.username && p.username.trim().toLowerCase() === state.currentUser.username.trim().toLowerCase()) return true;
            if (matchesCustomerName(p.customer || p.customerName || p.supplier || p.createdBy || p.owner, myCustNames)) {
                return true;
            }
            const inStockOut = (state.stockOutLogs || []).some(l => l && matchesCustomerName(l.customer, myCustNames) && l.code === p.code);
            const inBooking = (state.bookings || []).some(b => b && matchesCustomerName(b.customerName || b.customer, myCustNames) && b.productCode === p.code);
            return inStockOut || inBooking;
        });
    }
    ...
    if (isAdmin) {
        ...
        if (storeFilter) {
            ...
            return state.products.filter(p => {
                if (!p) return false;
                const sup = (p.supplier || p.username || '').toLowerCase().trim();
    ...
    const curUsername = state.currentUser.username.trim().toLowerCase();

    return state.products.filter(p => {
        if (!p) return false;
        if (!p.supplier || p.supplier.trim() === '' || p.supplier === 'ទូទៅ') return true;
        if (p.username && p.username.trim().toLowerCase() === curUsername) return true;
        if (matchesStoreName(p.supplier || p.customer || p.owner || p.createdBy, myStoreNames)) return true;
        
        const inStockIn = (state.stockInLogs || []).some(l => l && ((l.username && l.username.trim().toLowerCase() === curUsername) || matchesStoreName(l.supplier, myStoreNames)) && l.code === p.code);
        const inStockOut = (state.stockOutLogs || []).some(l => l && ((l.username && l.username.trim().toLowerCase() === curUsername) || matchesStoreName(l.supplier || l.customer, myStoreNames)) && l.code === p.code);
        const inBooking = (state.bookings || []).some(b => b && ((b.username && b.username.trim().toLowerCase() === curUsername) || matchesStoreName(b.customerName || b.staffName, myStoreNames)) && b.productCode === p.code);
        return inStockIn || inStockOut || inBooking;
    });
}

function getStoreFilteredStockInLogs() {
    if (!state.stockInLogs || !Array.isArray(state.stockInLogs)) return [];
    if (!state.currentUser) return state.stockInLogs;

    if (isCustomerRoleUser()) return [];
    ...
    if (isAdmin) {
        ...
        if (storeFilter) {
            ...
            return state.stockInLogs.filter(l => {
                const sup = (l.supplier || l.username || '').toLowerCase();
    ...
    return state.stockInLogs.filter(l => {
        if (!l) return false;
        if (!l.supplier || l.supplier.trim() === '' || l.supplier === 'ទូទៅ') return true;
        if (l.username && l.username.trim().toLowerCase() === curUsername) return true;
        return matchesStoreName(l.supplier || l.staffName, myStoreNames);
    });
}

function getStoreFilteredStockOutLogs() {
    if (!state.stockOutLogs || !Array.isArray(state.stockOutLogs)) return [];
    if (!state.currentUser) return state.stockOutLogs;

    if (isCustomerRoleUser()) {
        const myCustNames = getLoggedCustomerNames();
        const curUsername = state.currentUser.username.trim().toLowerCase();
        return state.stockOutLogs.filter(l => l && ((l.username && l.username.trim().toLowerCase() === curUsername) || matchesCustomerName(l.customer, myCustNames)));
    }
    ...
    if (isAdmin) {
        ...
        if (storeFilter) {
            ...
            return state.stockOutLogs.filter(l => {
                const sup = (l.supplier || l.seller || l.username || '').toLowerCase();
                return sup && (sup === storeNameLower || sup.includes(storeNameLower) || storeNameLower.includes(sup));
            });
        }
    ...
    return state.stockOutLogs.filter(l => {
        ...
        if (!l.supplier || l.supplier.trim() === '' || l.supplier === 'ទូទៅ') return true;
        if (l.username && l.username.trim().toLowerCase() === curUsername) return true;
        return matchesStoreName(l.supplier || l.seller || l.staffName || l.customer, myStoreNames);
    });
}

function getStoreFilteredBookings() {
    if (!state.bookings || !Array.isArray(state.bookings)) return [];
    if (!state.currentUser) return state.bookings;

    if (isCustomerRoleUser()) {
        const myCustNames = getLoggedCustomerNames();
        const curUsername = state.currentUser.username.trim().toLowerCase();
        return state.bookings.filter(b => b && ((b.username && b.username.trim().toLowerCase() === curUsername) || matchesCustomerName(b.customerName || b.customer, myCustNames)));
    }
    ...
    if (isAdmin) {
        ...
        if (storeFilter) {
            ...
            return state.bookings.filter(b => {
                const sup = (b.supplier || b.staffName || b.customerName || b.username || '').toLowerCase();
                return sup && (sup === storeNameLower || sup.includes(storeNameLower) || storeNameLower.includes(sup));
            });
        }
    ...
    return state.bookings.filter(b => {
        if (!b) return false;
        if (b.username && b.username.trim().toLowerCase() === curUsername) return true;
        return matchesStoreName(b.staffName || b.customerName || b.supplier, myStoreNames);
    });
}
    ...
    if (prodTfoot) {
        prodTfoot.innerHTML = `
            <tr class="table-summary-row">
                <td colspan="4" class="text-end fw-bold text-dark">សរុបទិន្នន័យចម្រោះ (${filtered.length} ទំនិញ)៖</td>
...
function populateSuppliersDatalist() {
    ...
    if (supplierFilter) {
        ...
        } else if (isCustomerRoleUser()) {
            const myCustName = (state.currentUser.fullName || state.currentUser.username || '').trim();
            supplierFilter.innerHTML = `<option value="${myCustName}">${myCustName}</option>`;
            supplierFilter.value = myCustName;
            supplierFilter.disabled = true;
        } else {
...
function handleProductFormSubmit(e) {
    ...
    const productObj = { code, name, category, cost, price, qty, minAlert, unit, supplier, username: state.currentUser ? state.currentUser.username : '' };
...
function completeStockInTx() {
    ...
    state.stockInCart.forEach(item => {
        ...
        const logObj = {
            ...
            staffName: state.currentUser ? (state.currentUser.fullName || state.currentUser.username) : ''
        };
...
function renderBookingsTable() {
    ...
    const baseBookings = getStoreFilteredBookings();

    const pendingCount = baseBookings.filter(b => b && b.status === 'Pending').length;
    const completedCount = baseBookings.filter(b => b && b.status === 'Completed').length;
    ...
    let filtered = baseBookings.filter(b => {
        if (!b) return false;
