...
function getStoreFilteredProducts() {
    ...
    // Strict isolation for Customer role users: ONLY see their own products / bookings / transactions!
    if (isCustomerRoleUser()) {
        const myCustNames = getLoggedCustomerNames();
        return state.products.filter(p => {
            if (!p) return false;
            // Matches if product supplier/customer/createdBy/owner matches logged customer name
            if (matchesCustomerName(p.customer || p.customerName || p.supplier || p.createdBy || p.owner, myCustNames)) {
                return true;
            }
            // Or if product is in the customer's own stock-out or booking logs
    ...
    // Admin / Manager / Cashier: filter by dropdown if selected
