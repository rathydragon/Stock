...
const defaultSeedData = {
    ...
    users: [
        { username: 'admin', password: '123456', fullName: 'អ្នកគ្រប់គ្រងប្រព័ន្ធ (Admin)', role: 'Admin', status: 'Active', allowedPages: ['dashboard', 'products', 'stock-in', 'stock-out', 'contacts', 'bookings', 'reports', 'settings'] },
        { username: 'store', password: '123456', fullName: 'ម្ចាស់ហាង (Store Owner)', role: 'Store', status: 'Active', allowedPages: ['dashboard', 'products', 'stock-in', 'stock-out', 'reports'] },
        { username: 'manager', password: '123456', fullName: 'អ្នកគ្រប់គ្រងស្តុក (Manager)', role: 'Manager', status: 'Active', allowedPages: ['dashboard', 'products', 'stock-in', 'stock-out', 'contacts', 'bookings', 'reports'] },
        { username: 'cashier', password: '123456', fullName: 'បុគ្គលិករៀបចំការលក់ (Cashier)', role: 'Cashier', status: 'Active', allowedPages: ['stock-out'] },
        { username: 'customer', password: '123456', fullName: 'អតិថិជន (Customer)', role: 'Customer', status: 'Active', allowedPages: ['stock-out'] }
    ],
    bookings: [
...
function getStoreFilteredProducts() {
    ...
    const storeFilter = document.getElementById('productSupplierFilter') ? document.getElementById('productSupplierFilter').value : '';

    const targetStore = isStoreUser ? getUserStoreName() : storeFilter;

    if (targetStore) {
        const targetLower = targetStore.toLowerCase().trim();
        const currentUsername = (state.currentUser.username || '').toLowerCase().trim();
        const linkedSuppliers = (state.suppliers || [])
            .filter(s => s.username && s.username.toLowerCase().trim() === currentUsername)
        ...
        return state.products.filter(p => {
            ...
            // Strict match: supplier equals or contains store name, username, or linked supplier names
            if (sup === targetLower || sup.includes(targetLower) || targetLower.includes(sup)) return true;
            if (currentUsername && (sup === currentUsername || sup.includes(currentUsername))) return true;
            if (linkedSuppliers.some(ls => ls && (sup === ls || sup.includes(ls) || ls.includes(sup)))) return true;
...
function getStoreFilteredStockInLogs() {
    ...
    const storeFilter = document.getElementById('productSupplierFilter') ? document.getElementById('productSupplierFilter').value : '';
    const targetStore = (userRole === 'Store' || userRole === 'Seller') ? getUserStoreName() : storeFilter;

    if (targetStore) {
        const storeProductCodes = new Set(storeProducts.map(p => p.code));
        const storeNameLower = targetStore.toLowerCase().trim();
        return (state.stockInLogs || []).filter(l => {
            if (storeProductCodes.has(l.code)) return true;
...
function populateSuppliersDatalist() {
    ...
    if (supplierFilter) {
        if (isStoreRoleUser()) {
            const myStore = getUserStoreName();
            supplierFilter.innerHTML = `<option value="${myStore}" selected>ម្ចាស់ហាង: ${myStore}</option>`;
            supplierFilter.disabled = true;
        } else {
            supplierFilter.disabled = false;
            const curVal = supplierFilter.value;
            supplierFilter.innerHTML = `<option value="">គ្រប់ម្ចាស់ហាង (Stores)</option>` +
    ...
    if (modalSupplierSelect) {
        if (isStoreRoleUser()) {
            const myStore = getUserStoreName();
            modalSupplierSelect.innerHTML = `<option value="${myStore}" selected>${myStore}</option>`;
            modalSupplierSelect.value = myStore;
...
function openProductModal(editIndex = -1) {
    ...
    if (editIndex >= 0) {
        ...
        if (document.getElementById('prodSupplier')) {
            ...
            if (isStoreRoleUser()) {
                document.getElementById('prodSupplier').disabled = true;
            }
        }
    } else {
        ...
        if (document.getElementById('prodSupplier')) {
            if (isStoreRoleUser()) {
                const myStore = getUserStoreName();
                document.getElementById('prodSupplier').value = myStore;
                document.getElementById('prodSupplier').disabled = true;
            } else {
...
function handleProductFormSubmit(e) {
    ...
    let supplier = document.getElementById('prodSupplier') ? (document.getElementById('prodSupplier').value || 'ទូទៅ') : 'ទូទៅ';
    if (isStoreRoleUser()) {
        supplier = getUserStoreName();
    }
...
function initStockInPanel() {
    ...
    if (supSelect) {
        if (isStoreRoleUser()) {
            const myStore = getUserStoreName();
            supSelect.innerHTML = `<option value="${myStore}" selected>ម្ចាស់ហាង: ${myStore}</option>`;
            ...
            supSelect.innerHTML = '<option value="">-- គ្រប់ម្ចាស់ហាងទាំងអស់ --</option>' +
                suppliersList.map(s => `<option value="${s}" ${s === curVal ? 'selected' : ''}>${s}</option>`).join('');
        }
    }
...
function handleLogout() {
    if (confirm('តើអ្នកពិតជាចង់ចាកចេញពីប្រព័ន្ធ ឬ?')) {
        state.currentUser = null;
...
function renderUsersTable() {
    ...
    if (state.users.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted p-3">ពុំទាន់មានគណនីប្រើប្រាស់នៅឡើយទេ</td></tr>';
        return;
    }
...
function handleUserFormSubmit(e) {
    ...
    const userObj = { username, password, fullName, role, status, allowedPages };
